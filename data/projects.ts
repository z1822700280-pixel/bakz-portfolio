...
export const projects: Project[] = [
  ...
  // ── ⑤ OpenCV 代码开发（待补充）──
  {
    id: 'face-motion-recognition',
    title: { zh: '面部和动作识别程序', en: 'Face & Motion Recognition' },
    description: {
      zh: '《情绪大活面》——基于 AI 摄像头的实时表情识别互动游戏，使用 Python + OpenCV + MediaPipe 构建，通过面部 468 个关键点、双手各 21 个关键点、身体 33 个关键点的实时检测，实现手势指向交互与表情相似度匹配两大核心玩法。\n\n项目包含闯关和休闲两种模式：闯关模式中，玩家需在 60 秒内通过模仿屏幕上的表情图片来获得分数，系统通过 Blendshapes 面部表情向量与余弦相似度算法实时计算匹配度，连续 15 帧超过 0.55 阈值则判定成功；休闲模式中，系统自动切换图片，用户自由体验。所有交互通过食指指向按钮并保持 3 秒触发，无需鼠标键盘。\n\n技术实现上，我封装了 MediaPipe 三个视觉模型的统一检测接口，设计了基于 Blendshapes 的 20 维面部特征向量、手部关节角度与身体姿态角度的多模态特征融合匹配算法，并用 pygame-ce 实现了三场景独立 BGM 与音效系统。\n\n开发过程中我独立解决了 MediaPipe 时间戳冲突导致的崩溃问题、状态机转换卡死、首帧加载卡顿、无限递归等多个技术难点。最终程序运行稳定在 25-30 FPS，10 项测试用例全部通过。本项目为 VR 平台开发基础课程设计，由我独立完成全部开发工作。',
      en: 'Emotion Expression Game — An AI camera-based real-time facial expression recognition interactive game built with Python + OpenCV + MediaPipe. It features real-time detection of 468 facial landmarks, 21 hand landmarks per hand, and 33 body landmarks, enabling gesture-based pointing interaction and expression similarity matching.\n\nThe game includes two modes: Challenge mode requires players to match 10 reference expressions within 60 seconds. The system uses Blendshapes facial vectors and cosine similarity algorithms for real-time matching, requiring 15 consecutive frames above a 0.55 threshold. Relax mode automatically cycles through images for free-form experience. All interactions are triggered by pointing an index finger at buttons for 3 seconds — no mouse or keyboard needed.\n\nTechnically, I wrapped MediaPipe\'s three vision models into a unified detection interface, designed a multi-modal feature fusion matching algorithm combining 20-dimensional Blendshapes facial vectors, hand joint angles, and body posture angles, and implemented a three-scene BGM system with pygame-ce.\n\nDuring development, I independently solved several technical challenges including MediaPipe timestamp conflicts causing crashes, state machine transition deadlocks, first-frame loading lag, and infinite recursion. The final program runs stably at 25-30 FPS, passing all 10 test cases. This project was completed independently as a VR Platform Development course design.',
    },
    category: 'opencv',
    tags: ['OpenCV', 'MediaPipe', '面部识别', '手势识别', 'Python', '表情匹配', '实时交互'],
    year: 2026,
    thumbnail: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/thumbnail.png',
    images: [
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/demo.mp4',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/content-code-1.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/content-code-2.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/content-code-3.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/content-code-4.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/process-1.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/process-2.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/process-3.png',
    ],
    tools: ['Python 3.14', 'OpenCV 4.13', 'MediaPipe 0.10', 'Pygame-CE', 'NumPy'],
    role: { zh: '独立开发 / 全流程', en: 'Solo Developer / Full Pipeline' },
    externalLinks: [],
    reflection: {
      zh: '本次《情绪大活面》项目是我在 VR 平台开发基础课程中独立完成的课程设计作品。这是我第一次将计算机视觉技术应用到完整的游戏交互项目中，从需求分析到最终交付全部由我一人完成，让我对 OpenCV 和 MediaPipe 的实际应用有了系统性的掌握。\n\n项目中最具挑战性的部分是实现准确的实时表情匹配。我最初尝试使用几何比例特征（嘴宽/嘴高、眉高等）来计算面部相似度，但在测试中发现这些指标对微表情变化不够敏感。后来我改用 MediaPipe 的 Blendshapes（20 维面部表情向量），再配合手部关节角度和身体姿态角度进行加权融合（face×0.4 + hand×0.3 + pose×0.3），匹配准确率有了质的变化。\n\n开发中遇到的四个关键问题：一是 MediaPipe 时间戳冲突——闯关模式加载参考图片时使用了独立计数器，导致视频帧检测崩溃，统一使用全局时序计数器解决；二是状态转换卡死——get_pointing_progress() 每帧被错误调用三次导致内部状态被提前清除，改为每帧只调用一次并缓存结果修复；三是首帧加载卡顿——同步加载参考图片导致界面冻结 5-10 秒，改用懒加载策略，在 update_challenge() 中异步执行并显示 LOADING 提示；四是无限递归——所有图片均无法检测特征时的递归栈溢出，改为迭代方式并设置最大尝试次数。\n\n这次实践让我深刻体会到：计算机视觉项目不仅要懂算法，更需要处理工程落地中的各种边界情况和异常流程。同时，我掌握了 MediaPipe 视觉模型的实际使用、OpenCV 实时绘制技术、有限状态机设计模式在游戏中的应用，以及 Python 项目从开发到测试的完整流程。',
      en: 'Emotion Expression Game was my independently completed course design for VR Platform Development. This was my first time applying computer vision technology to a complete interactive game project, covering the full pipeline from requirements analysis to final delivery entirely by myself, giving me systematic mastery of OpenCV and MediaPipe in practice.\n\nThe most challenging part was achieving accurate real-time expression matching. I initially tried geometric ratio features (mouth width/height, eyebrow height, etc.) for facial similarity calculation, but tests showed these metrics were insufficiently sensitive to subtle expression changes. I switched to MediaPipe\'s Blendshapes (20-dimensional facial expression vectors) combined with hand joint angles and body posture angles using weighted fusion (face×0.4 + hand×0.3 + pose×0.3), which dramatically improved matching accuracy.\n\nFour key technical challenges were encountered: First, MediaPipe timestamp conflicts — loading reference images in Challenge mode used a separate counter, causing video frame detection crashes. Solved by using a unified global timestamp counter. Second, state transition deadlock — get_pointing_progress() was called three times per frame, clearing internal state prematurely. Fixed by calling it once per frame and caching results. Third, first-frame loading lag — synchronous image loading froze the interface for 5-10 seconds. Adopted lazy loading with async execution in update_challenge() showing a LOADING indicator. Fourth, infinite recursion — when all reference images lacked detectable features, recursive calls caused stack overflow. Converted to iterative approach with maximum retry limit.\n\nThis project taught me that computer vision development requires not only algorithmic knowledge but also robust handling of edge cases and abnormal flows in engineering practice. I gained hands-on experience with MediaPipe vision models, OpenCV real-time rendering, finite state machine design patterns in games, and the complete Python development-to-testing workflow.',
    },
    snapshot: [
      { label: '项目类型', value: '实时表情识别互动游戏' },
      { label: '技术栈', value: 'Python 3.14 / OpenCV 4.13 / MediaPipe 0.10' },
      { label: '核心能力', value: '面部468点 + 双手21点 + 身体33点实时检测' },
      { label: '交互方式', value: '手势指向（食指3秒触发），无需键鼠' },
      { label: '匹配算法', value: 'Blendshapes 20维向量 + 余弦相似度加权融合' },
      { label: '运行帧率', value: '25-30 FPS，延迟 < 100ms' },
      { label: '测试结果', value: '10项测试全部通过' },
      { label: '开发周期', value: '2026年5月' },
    ],
    pipeline: [
      {
        title: '项目规划与需求分析',
        target: '明确项目目标与功能范围',
        work: '分析课程设计要求，确定"基于 AI 摄像头的实时表情识别互动游戏"方向。梳理核心功能：摄像头实时检测、手势指向交互、表情相似度匹配、闯关与休闲双模式、音效反馈系统。规划项目文件结构与模块划分。',
        problems: '项目周期仅一个月，需要合理控制功能范围',
        solutions: '优先实现核心交互闭环，音效等辅助功能使用成熟库（pygame-ce）快速集成',
      },
      {
        title: '环境搭建与技术选型',
        target: '搭建开发环境，确定技术方案',
        work: '安装 Python 3.14 及依赖库（OpenCV 4.13、MediaPipe 0.10、NumPy 2.4、pygame-ce 2.5）。下载 MediaPipe Tasks 预训练模型（面部 3.58MB、手部 7.46MB、姿态 5.51MB）。搭建项目目录结构，验证摄像头采集与模型加载是否正常。',
        problems: 'MediaPipe 各版本 API 差异较大，文档不够清晰',
        solutions: '参考官方示例代码，选择 tasks-vision API 作为统一接口',
      },
      {
        title: 'MediaPipe 统一检测封装',
        target: '封装三个视觉模型的统一调用接口',
        work: '编写 MPManager 类，统一封装 FaceLandmarker、HandLandmarker、PoseLandmarker 三个模型。所有模型使用同一全局时序计数器（_ts），采用 VIDEO 模式实现逐帧检测。detect() 方法返回包含面部 468 关键点、双手各 21 关键点、身体 33 关键点的统一字典。',
        problems: '三个模型独立初始化耗时较长，启动需 3-5 秒',
        solutions: '启动时并行初始化，添加加载进度提示',
      },
      {
        title: '特征提取与匹配算法开发',
        target: '实现基于 Blendshapes 的多模态表情相似度匹配',
        work: '开发 FeatureMatcher 类：面部特征优先使用 Blendshapes 的 20 维表情向量（嘴笑/嘴嘟/眉高/眼开合等），回退方案为 10 维几何比例特征（嘴宽高比、眉高、眼开合、鼻嘴距离等）。手部特征取左右手最佳匹配结果，姿态特征为 9 维身体角度向量。最终加权融合：face×0.4 + hand×0.3 + pose×0.3，输出总体相似度。',
        problems: '仅用几何比例时，微表情变化不够敏感',
        solutions: '改用 Blendshapes 20 维向量作为主要面部特征，准确率显著提升',
      },
      {
        title: '游戏状态机开发',
        target: '实现五状态有限状态机与手势指向交互',
        work: '开发 Game 类管理 MENU / COUNTDOWN / CHALLENGE / RELAX / RESULT 五个状态及其转换逻辑。实现食指指向检测：通过食指尖（landmark 8）与 PIP 关节（landmark 6）的 Y 坐标判断手指是否伸直，再检测指尖是否落在按钮区域。指向保持 3 秒触发进入，每帧只调用一次 get_pointing_progress() 避免状态紊乱。',
        problems: 'get_pointing_progress() 每帧被调用 3 次导致内部状态提前清除，start_countdown() 永远无法触发',
        solutions: '改为每帧只调用一次，缓存结果复用；修复后状态转换完全正常',
      },
      {
        title: '闯关模式核心逻辑',
        target: '60 秒限时匹配 10 张参考表情图片',
        work: '闯关模式匹配判定需连续 15 帧相似度超过阈值 0.55 才判定成功，防止误触发。匹配成功后加载下一张图片（懒加载，异步执行避免卡顿）。60 秒超时或 10 张全部完成进入结果界面。所有图片均无法检测特征时设置最大尝试次数（图片数×2），超限后返回主界面并打印警告。',
        problems: '同步加载参考图片导致界面冻结 5-10 秒；无限递归导致栈溢出',
        solutions: '懒加载：start_challenge() 仅初始化状态，update_challenge() 中异步执行并显示 LOADING；迭代代替递归，设最大尝试次数',
      },
      {
        title: 'UI 绘制与视觉反馈',
        target: '通过 OpenCV 绘制实时视觉反馈界面',
        work: '集中所有 OpenCV 绘制逻辑在 ui.py 中。实现：面部边界框与关键点绘制（每 5 个取 1 个）、手部骨架（21 个关键点 + 连接线）、姿态骨架（33 个关键点 + 连接线）、参考图片叠加（画面右侧缩放显示）、按钮区域与指向进度圆圈、相似度实时数值显示、倒计时与分数显示。',
        problems: '绘制内容较多时帧率下降明显',
        solutions: '关键点采样绘制（每 5 取 1），降低绘制开销；使用 cv2.rectangle 和 cv2.line 替代逐像素操作',
      },
      {
        title: '音效系统集成',
        target: '实现三场景独立 BGM 与即时 SFX 触发',
        work: '使用 pygame-ce 开发 SoundManager 类：BGM 循环播放，同一首不重复加载；SFX 缓存已加载音效避免重复 IO；状态切换时自动更换 BGM（COUNTDOWN 和 RESULT 时停止）；SFX 文件缺失时静默跳过不报错。',
        problems: '首次加载音效时因文件 IO 导致短暂卡顿',
        solutions: '启动时预加载所有音效文件到缓存，运行时直接从内存播放',
      },
      {
        title: '测试调试与优化',
        target: '全面测试各功能模块并修复边界问题',
        work: '设计 10 项测试用例覆盖全部功能：摄像头采集、面部检测、手部检测、姿态检测、按钮指向交互、闯关模式流程、休闲模式流程、返回按钮、音效播放、空素材容错。逐项测试并记录结果。',
        problems: '空素材目录时程序未做容错处理直接崩溃',
        solutions: '添加空素材检测，清空 materials 后提示警告并返回主界面，setdefault.png 不存在时跳过绘制',
      },
    ],
    challenges: [
      {
        question: 'MediaPipe 时间戳冲突导致程序崩溃——detect_image() 与 detect() 时间戳不连续',
        solutions: [
          'detect() 和 detect_image() 统一使用同一个全局 _ts 计数器，确保时间戳全局单调递增',
          '所有 MediaPipe 模型调用均传入同一个 timestamp 参数，避免时间回溯',
          '将计数器从每次调用加 1 改为使用系统时间戳毫秒值，彻底避免冲突',
        ],
      },
      {
        question: '状态转换卡死——get_pointing_progress() 状态被多帧并发调用提前清除',
        solutions: [
          '重构为每帧只调用一次 get_pointing_progress()，将结果存入实例变量供后续复用',
          '添加调用次数断言（assert），开发阶段检测意外的重复调用',
          '状态机内部状态变更操作加锁保护，确保原子性',
        ],
      },
      {
        question: '首帧加载卡顿——同步加载参考图片导致界面冻结 5-10 秒',
        solutions: [
          '采用懒加载策略：start_challenge() 仅初始化状态变量，不执行图片加载',
          '将图片加载推迟到 update_challenge() 中异步执行，期间显示 "LOADING..." 提示',
          '预加载下一张图片，用户匹配当前图片时提前准备下一张',
        ],
      },
      {
        question: '无限递归——所有参考图片均无法检测有效特征时递归栈溢出',
        solutions: [
          '将递归调用改为迭代循环，使用 while 循环替代函数自调用',
          '设置最大尝试次数为图片数量的 2 倍，超限后返回主界面并打印警告日志',
          '添加 fallback 逻辑：指定次数后仍无有效图片则进入无图模式，跳过匹配直接展示',
        ],
      },
    ],
    overviewSections: [
      {
        title: '项目背景',
        content: '《情绪大活面》是 VR 平台开发基础课程的课程设计作品，由我独立完成全部开发工作。项目旨在利用 MediaPipe 的面部、手部和姿态检测能力，构建一个基于摄像头的实时表情识别互动游戏，让用户通过模仿屏幕上的表情图片来获得分数，体验 AI 驱动的趣味互动。项目周期为 2026 年 5 月，采用 Python 3.14 + OpenCV 4.13 + MediaPipe 0.10 + Pygame-CE 技术栈。',
      },
      {
        title: '核心功能',
        content: '项目包含闯关和休闲两种模式：闯关模式限时 60 秒匹配 10 张参考表情，需连续 15 帧相似度超过 0.55 阈值才算成功，匹配成功后自动加载下一张；休闲模式无时间限制，系统自动切换图片，相似度低于 0.3 时自动切换。所有交互通过食指指向屏幕按钮并保持 3 秒触发，无需鼠标键盘。三个场景（主界面/闯关/休闲）各自配有独立 BGM，匹配成功、超时失败、按钮悬停各有独立音效。',
      },
      {
        title: '技术实现',
        content: '系统由五个核心模块组成：MPManager 封装 MediaPipe 三个视觉模型的统一检测接口（面部 468 点 + 双手各 21 点 + 身体 33 点）；FeatureMatcher 实现基于 Blendshapes 20 维面部表情向量与余弦相似度的多模态加权匹配（face×0.4 + hand×0.3 + pose×0.3）；Game 类管理五状态有限状态机（MENU→COUNTDOWN→CHALLENGE/RELAX→RESULT→MENU）；ui.py 集中所有 OpenCV 绘制逻辑；SoundManager 基于 pygame-ce 实现 BGM 循环与 SFX 即时触发。',
      },
      {
        title: '遇到的问题与解决方案',
        content: '开发中遇到四个关键问题：（1）MediaPipe 时间戳冲突——detect_image() 使用独立计数器导致后续帧时间戳小于已记录值，统一全局 _ts 计数器解决；（2）状态转换卡死——get_pointing_progress() 每帧被错误调用三次导致内部状态被提前清除，改为每帧只调用一次并缓存结果；（3）首帧加载卡顿——同步加载图片冻结界面，采用懒加载异步执行并显示 LOADING 提示；（4）无限递归——所有图片均无法检测特征时递归栈溢出，改为迭代方式并设置最大尝试次数。',
      },
      {
        title: '测试与最终效果',
        content: '设计了 10 项测试用例覆盖全部功能模块（摄像头采集、面部/手部/姿态检测、按钮指向交互、闯关/休闲模式流程、返回按钮、音效播放、空素材容错），全部测试通过。程序在 Windows 11 + Python 3.14 环境下运行稳定，帧率维持在 25-30 FPS，帧处理延迟低于 100ms，满足实时交互需求。',
      },
    ],
    mediaItems: [
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/demo.mp4', type: 'video', title: '完整演示视频', description: '展示从主界面到闯关/休闲模式的完整游戏流程' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/content-code-1.png', type: 'image', title: 'MPManager 检测封装', description: 'MediaPipe 三个视觉模型的统一检测接口封装类' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/content-code-2.png', type: 'image', title: 'FeatureMatcher 特征提取', description: '基于 Blendshapes 20 维向量的多模态特征融合匹配算法' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/content-code-3.png', type: 'image', title: 'Game 状态机', description: '五状态有限状态机实现游戏模式切换逻辑' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/content-code-4.png', type: 'image', title: 'UI 绘制与手势交互', description: 'OpenCV 实时绘制面部框、手部骨架与指向交互逻辑' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/process-1.png', type: 'image', title: 'MediaPipe 检测效果', description: '面部 468 关键点 + 手部 21 关键点 + 身体 33 关键点实时检测' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/process-2.png', type: 'image', title: 'Blendshapes 可视化', description: '实时面部表情向量分析与相似度数值显示' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/face-motion-recognition/process-3.png', type: 'image', title: '测试结果', description: '10 项测试用例全部通过的测试记录' },
    ],
  },
  {
    id: 'denoising-morphological',
    title: { zh: '图像去噪与形态学处理', en: 'Image Denoising & Morphological Processing' },
    description: {
      zh: '《VR 平台开发基础》课程的实验二作业，主题是图像平滑与形态学处理。实验使用 OpenCV 和 NumPy，针对三种污染图像（高斯噪声、椒盐噪声、形态学污染），分别实现均值滤波、高斯滤波、中值滤波和形态学腐蚀/膨胀四种恢复方法，并搭建了带滑动条的交互界面，可以动态调节卷积核大小来观察处理效果。\n\n实验内容分为四部分：均值滤波搭建滑动条交互界面，动态调节核大小进行平滑去噪；高斯滤波调用 GaussianBlur 函数，限定核为奇数尺寸；中值滤波使用 medianBlur，针对椒盐噪声图像处理；形态学处理先用 NumPy 创建结构核，再基于 OpenCV 实现腐蚀和膨胀操作。所有实验都做了带滑动条的可视化交互，方便直观对比不同参数下处理效果的变化。\n\n通过这个实验，我基本掌握了 OpenCV 图像处理基础函数的调用方式，理解了不同去噪算法的原理和适用场景，也练习了参数调优的基本方法。实验代码中部分框架参考了 AI 辅助生成的示例，核心逻辑和参数调试由自己完成。',
      en: 'This is Experiment 2 of the VR Platform Development course, focusing on image smoothing and morphological processing. Using OpenCV and NumPy, I implemented four restoration methods (mean filter, Gaussian blur, median filter, and morphological erosion/dilation) for three types of corrupted images (Gaussian noise, salt-and-pepper noise, and morphological contamination). An interactive interface with sliders was built to dynamically adjust kernel sizes and observe processing effects in real time.\n\nThe experiment consisted of four parts: mean filtering with an interactive trackbar interface for kernel size adjustment; Gaussian blur using the GaussianBlur function with odd kernel sizes; median filtering using medianBlur for salt-and-pepper noise; and morphological processing using NumPy-created structuring kernels with OpenCV erosion and dilation operations. All experiments included slider-based visualization for intuitive parameter comparison.\n\nThrough this lab, I gained basic proficiency in calling OpenCV image processing functions, understood the principles and applicable scenarios of different denoising algorithms, and practiced parameter tuning fundamentals. Some code structure references were generated with AI assistance, while core logic and parameter tuning were completed independently.',
    },
    category: 'opencv',
    tags: ['OpenCV', '图像处理', '均值滤波', '高斯滤波', '中值滤波', '形态学', 'NumPy'],
    year: 2026,
    thumbnail: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/thumbnail.png',
    images: [
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-mean-1.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-mean-2.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-gaussian-1.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-gaussian-2.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-gaussian-3.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-median-1.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-median-2.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-median-3.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-1.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-2.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-3.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-4.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-5.png',
    ],
    tools: ['Python', 'OpenCV', 'NumPy'],
    role: { zh: '独立完成实验', en: 'Independent Lab Work' },
    externalLinks: [],
    reflection: {
      zh: '这是《VR 平台开发基础》课程的一次实验作业，主要练习图像平滑处理和形态学操作的基本方法。通过这次实验，我对 OpenCV 的基础图像处理函数有了初步掌握，也理解了不同滤波算法的基本区别。\n\n四种算法各有特点：均值滤波实现最简单，运算快，但容易让图像变模糊；高斯滤波对高斯噪声效果最好，能保留更多纹理；中值滤波对付椒盐噪声很有效，边缘保护也好；形态学的腐蚀和膨胀则是专门处理结构型噪声的。实验中通过滑动条调参数，直观感受到了卷积核大小对处理结果的影响——核太大细节就没了，核太小又去不干净。\n\n实验过程中也遇到一些小问题，比如滑动条的参数范围设置不合理导致图像过度模糊，调整了几次范围才找到合适的区间。整体来说是一次扎实的基础练习，让我对图像处理的实际操作有了更具体的认识。',
      en: 'This was a lab assignment for the VR Platform Development course focusing on basic image smoothing and morphological processing methods. Through this experiment, I gained initial familiarity with OpenCV\'s basic image processing functions and understood the key differences between various filtering algorithms.\n\nEach of the four algorithms has distinct characteristics: mean filtering is the simplest with fast computation but tends to blur images; Gaussian blur works best for Gaussian noise while preserving more texture; median filtering is effective against salt-and-pepper noise with good edge preservation; morphological erosion and dilation are specifically designed for structural noise. Using the slider-based interface, I could intuitively observe how kernel size affects results — too large and details disappear, too small and noise remains.\n\nI encountered some minor issues during the experiment, such as setting inappropriate slider ranges that caused excessive blurring, which required several adjustments to find optimal parameters. Overall, this was a solid foundational exercise that gave me practical hands-on experience with image processing operations.',
    },
    snapshot: [
      { label: '项目类型', value: '课程实验（VR平台开发基础）' },
      { label: '实验内容', value: '均值滤波 / 高斯滤波 / 中值滤波 / 形态学处理' },
      { label: '技术栈', value: 'Python / OpenCV / NumPy' },
      { label: '交互方式', value: '滑动条动态调节卷积核参数' },
      { label: '处理对象', value: '高斯噪声 / 椒盐噪声 / 形态学污染图像' },
      { label: '实验日期', value: '2026年3月' },
    ],
    pipeline: [
      {
        title: '环境准备与理解实验要求',
        target: '明确实验目的和四种算法的作用场景',
        work: '阅读实验指导书，理解三种污染图像（高斯噪声、椒盐噪声、形态学污染）的特点，以及对应的恢复方法。在 Python 环境中安装 opencv-python 和 numpy 依赖库。',
        problems: '刚开始对四种滤波算法的区别理解不够清晰',
        solutions: '先分别跑通每个算法的示例代码，观察输出结果，再对比理解各自适用场景',
      },
      {
        title: '均值滤波实验',
        target: '实现均值滤波并验证去噪效果',
        work: '基于 OpenCV 的 blur 函数实现均值滤波，搭建滑动条交互界面，动态调节卷积核大小（从 3×3 到 21×21），对高斯噪声和椒盐噪声污染图像进行平滑去噪处理。',
        problems: '核尺寸太大时图像边缘模糊严重，细节丢失明显',
        solutions: '对比不同核尺寸的效果，找到去噪效果和细节保留的平衡点',
      },
      {
        title: '高斯滤波实验',
        target: '实现高斯滤波并评估对高斯噪声的去除效果',
        work: '调用 GaussianBlur 函数，限定核为奇数尺寸（3×3、5×5、7×7 等），通过滑动条调节参数，分别处理高斯噪声图像。',
        problems: '高斯滤波的参数（核尺寸和 sigma 值）需要配合调整才能达到最佳效果',
        solutions: '固定 sigma 值后调节核尺寸，观察不同组合的处理效果',
      },
      {
        title: '中值滤波实验',
        target: '实现中值滤波并验证对椒盐噪声的针对性修复能力',
        work: '使用 medianBlur 中值滤波函数，通过滑动条调节核大小（从 3×3 到 15×15），针对椒盐噪声图像进行去噪处理。',
        problems: '中值滤波对大面积的椒盐噪声处理不彻底',
        solutions: '适当增大核尺寸，但注意核太大会影响图像整体清晰度',
      },
      {
        title: '形态学处理实验',
        target: '实现腐蚀和膨胀操作，修复形态学污染图像',
        work: '先用 NumPy 创建不同形状的结构核（矩形、十字形等），再基于 OpenCV 的 erode 和 dilate 函数实现腐蚀与膨胀操作。通过滑动条调节结构核大小，观察不同参数下的形态学处理效果。',
        problems: '结构核的形状和大小对处理结果影响很大，不太容易一次调对',
        solutions: '分别用矩形核和十字形核做对比测试，选择效果更好的组合',
      },
      {
        title: '结果整理与实验报告',
        target: '汇总各算法的处理效果，完成实验报告',
        work: '对比四种算法在不同噪声类型上的处理效果，截图保存关键结果，撰写实验报告文档，总结各算法的适用场景和参数经验。',
      },
    ],
    challenges: [
      {
        question: '均值滤波核尺寸如何选择才能平衡去噪与细节保留？',
        solutions: [
          '核尺寸过小（3×3）去噪效果有限，核尺寸过大（>15×15）细节模糊严重',
          '通过滑动条实时对比不同尺寸的效果，根据噪声严重程度灵活选择',
          '实验中发现 7×7 到 11×11 是比较实用的范围',
        ],
      },
      {
        question: '不同滤波算法对不同类型噪声的处理效果差异在哪里？',
        solutions: [
          '高斯滤波对高斯噪声效果最优，平滑过渡自然',
          '中值滤波对椒盐噪声有针对性修复能力，且能保护边缘',
          '均值滤波对两种噪声都有一定效果，但整体最粗糙',
        ],
      },
      {
        question: '形态学处理中结构核的选择对结果有何影响？',
        solutions: [
          '矩形核腐蚀效果更强、范围更广，但可能过度消除细节',
          '十字形核更精细，适合处理细长型的结构问题',
          '需要根据具体污染形态选择合适的结构核形状和尺寸',
        ],
      },
    ],
    overviewSections: [
      {
        title: '实验背景',
        content: '本实验是《VR 平台开发基础》课程的第二次实验，主题为图像平滑与形态学处理。实验目的是掌握使用平滑处理及形态学方法进行图像噪声恢复的基本方法。实验基于 Python + OpenCV + NumPy 技术栈，在 PC 机上进行开发与测试。',
      },
      {
        title: '实验内容',
        content: '实验给定三种污染图像（高斯噪声、椒盐噪声、形态学污染），要求使用 OpenCV 和 NumPy 实现对应的恢复函数。具体包括四个子实验：均值滤波（通过滑动条动态调节卷积核大小进行平滑去噪）、高斯滤波（调用 GaussianBlur 函数处理高斯噪声图像）、中值滤波（使用 medianBlur 针对椒盐噪声处理）、形态学处理（基于 NumPy 创建结构核，实现腐蚀与膨胀操作）。所有子实验均搭建了滑动条交互界面，可实时观察参数变化对处理效果的影响。',
      },
      {
        title: '实验结果分析',
        content: '通过对比四种算法的处理效果：均值滤波实现简单、运算速度快，可弱化常规噪声，但易导致图像细节模糊；高斯滤波对高斯噪声的抑制效果最优，平滑自然且保留更多纹理信息；中值滤波对椒盐噪声具有针对性修复能力，能在去噪同时保护边缘；形态学腐蚀与膨胀操作则专门用于结构型污染修复，可消除细小干扰、填补图像缺口。实验还验证了卷积核尺寸是影响处理效果的核心参数。',
      },
      {
        title: '实验总结',
        content: '通过本次实验，我熟练掌握了 OpenCV 图像处理基础函数的调用方式，理解了不同去噪算法的原理与适用场景，提升了基本的参数调优能力。同时认识到图像处理需要根据噪声类型选择匹配的算法，合理的参数设置是保证修复质量的关键。实验代码中部分框架性代码参考了 AI 辅助工具，核心算法理解和参数调试由自己独立完成。',
      },
    ],
    mediaItems: [
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-mean-1.png', type: 'image', title: '均值滤波实验', description: '基于 blur 函数实现均值滤波，滑动条调节卷积核大小' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-mean-2.png', type: 'image', title: '均值滤波效果对比', description: '不同核尺寸下的去噪效果与细节保留对比' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-gaussian-1.png', type: 'image', title: '高斯滤波实验', description: '调用 GaussianBlur 函数处理高斯噪声图像' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-gaussian-2.png', type: 'image', title: '高斯滤波效果', description: '高斯噪声去除效果展示，平滑过渡自然' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-gaussian-3.png', type: 'image', title: '高斯滤波参数调试', description: '不同 sigma 值与核尺寸组合的处理效果对比' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-median-1.png', type: 'image', title: '中值滤波实验', description: '使用 medianBlur 处理椒盐噪声图像' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-median-2.png', type: 'image', title: '中值滤波效果', description: '对椒盐噪声的针对性修复效果展示' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-median-3.png', type: 'image', title: '中值滤波参数对比', description: '不同核尺寸下椒盐噪声去除效果对比' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-1.png', type: 'image', title: '形态学处理实验', description: '基于 NumPy 创建结构核，实现腐蚀与膨胀操作' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-2.png', type: 'image', title: '腐蚀操作效果', description: '腐蚀可消除图像细小突出噪声' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-3.png', type: 'image', title: '膨胀操作效果', description: '膨胀操作能填补图像区域缺口' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-4.png', type: 'image', title: '形态学综合效果', description: '腐蚀与膨胀配合使用修复结构缺陷' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/denoising-morphological/exp-morph-5.png', type: 'image', title: '结构核对比', description: '矩形核与十字形核处理效果对比展示' },
    ],
  },
  {
    id: 'feature-detection',
    title: { zh: '图像特征检测', en: 'Image Feature Detection' },
    description: {
      zh: '《VR 平台开发基础》课程的实验四作业，主题是图像特征检测。实验使用 OpenCV 的 SIFT 算法提取图像特征点，FLANN 匹配器进行快速特征匹配，再结合 calib3d 模块的单应性变换，实现摄像头画面中教材封面（特定目标）的实时识别与定位标记。\n\n实验流程分四步：先用 SIFT 算法提取教材封面模板图和摄像头实时画面的关键特征点与描述子；然后用 FLANN 匹配器对两组特征进行快速匹配，设置距离阈值筛选优质匹配点；接着根据匹配点计算单应性变换矩阵，将模板轮廓坐标映射到摄像头画面中；最后绘制矩形框完成目标定位。\n\n实验最终实现了教材封面的实时识别与框选，在画面抖动和角度偏移场景下仍能正常识别，仅光线过暗时特征点减少、匹配精度略有下降。通过这次实验，我对 SIFT 特征检测、特征匹配和目标定位的基本流程有了直观理解。代码框架部分参考了 AI 辅助，核心实现和参数调试独立完成。',
      en: 'This is Experiment 4 of the VR Platform Development course, focusing on image feature detection. The experiment uses OpenCV\'s SIFT algorithm for feature point extraction, FLANN matcher for fast feature matching, and calib3d module\'s homography transformation for real-time textbook cover recognition and localization in camera feed.\n\nThe workflow consists of four steps: first, extract key feature points and descriptors from both the textbook cover template and the live camera feed using SIFT; then use FLANN matcher for fast feature matching with distance threshold filtering; next, calculate the homography transformation matrix from matched points to map the template contour coordinates to the camera frame; finally, draw a bounding box for target localization.\n\nThe experiment successfully achieved real-time textbook cover recognition and box localization, functioning well under camera shake and angle changes, with only minor accuracy degradation in low-light conditions. Through this lab, I gained practical understanding of SIFT feature detection, feature matching, and object localization workflows. Code structure was partially AI-assisted, with core implementation and parameter tuning completed independently.',
    },
    category: 'opencv',
    tags: ['OpenCV', 'SIFT', '特征检测', 'FLANN', '单应性变换', '目标识别', 'Python'],
    year: 2026,
    thumbnail: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/feature-detection/thumbnail.jpeg',
    images: [
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/feature-detection/result-1.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/feature-detection/result-2.png',
      'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/feature-detection/result-3.png',
    ],
    tools: ['Python', 'OpenCV', 'NumPy'],
    role: { zh: '独立完成实验', en: 'Independent Lab Work' },
    externalLinks: [],
    reflection: {
      zh: '这是《VR 平台开发基础》课程的一次实验作业，练习基于 SIFT 算法的图像特征检测、FLANN 特征匹配及单应性变换的目标定位方法。通过这次实验，我对计算机视觉中特征提取和目标识别的基本流程有了直观的认识。\n\nSIFT 算法确实挺稳定的，不管图像怎么缩放、旋转、变换视角，特征点都能被检测到，这也是它作为经典算法的原因。FLANN 匹配器的匹配速度比暴力匹配快很多，适合实时画面处理。实验中调匹配阈值花了不少时间——阈值设太高匹配点太少，定位框出不来或位置偏；设太低又全是误匹配，框到处乱跳。来回试了几组值才找到一个相对稳定的区间。\n\n也遇到过定位框偏移的问题，后来发现是图像没有做预处理，光照变化太剧烈的时候特征点不稳定。简单调了一下图像预处理方式之后情况好了很多。整体来说是一次挺有收获的实践，让我对特征检测的实际应用有了更具体的了解。',
      en: 'This was a lab assignment for the VR Platform Development course focusing on SIFT-based image feature detection, FLANN feature matching, and homography-based object localization. Through this experiment, I gained practical understanding of feature extraction and object recognition workflows in computer vision.\n\nSIFT proved to be remarkably stable — regardless of image scaling, rotation, or perspective changes, feature points could still be detected, which explains its status as a classic algorithm. FLANN matcher was significantly faster than brute-force matching, making it suitable for real-time camera processing. Adjusting the matching threshold took considerable time — too high and too few matches caused inaccurate or missing bounding boxes; too low and false matches caused the box to jump erratically. It took several parameter iterations to find a stable range.\n\nI also encountered bounding box offset issues, which I traced back to inadequate image preprocessing —剧烈 lighting changes destabilized feature points. Simple preprocessing adjustments significantly improved results. Overall, this was a rewarding hands-on experience that gave me concrete understanding of feature detection applications.',
    },
    snapshot: [
      { label: '项目类型', value: '课程实验（VR平台开发基础）' },
      { label: '实验内容', value: 'SIFT特征检测 / FLANN匹配 / 单应性变换目标定位' },
      { label: '技术栈', value: 'Python / OpenCV / NumPy' },
      { label: '核心算法', value: 'SIFT (尺度不变特征变换)' },
      { label: '匹配方式', value: 'FLANN 快速最近邻匹配 + 距离阈值筛选' },
      { label: '实验日期', value: '2026年4月' },
    ],
    pipeline: [
      {
        title: '理解实验要求与搭建环境',
        target: '明确实验目标，配置开发环境',
        work: '阅读实验指导书，理解要求：使用 SIFT 算法检测教材封面特征，FLANN 匹配器匹配，单应性变换定位框选。在 Python 中安装 opencv-python（含 contrib 模块以支持 SIFT）和 numpy。准备教材封面图片作为目标模板。',
        problems: 'OpenCV 高版本中 SIFT 需要安装 opencv-contrib-python 才能使用',
        solutions: '安装 opencv-contrib-python 库，确认 SIFT_create() 函数可用',
      },
      {
        title: 'SIFT 特征检测',
        target: '提取模板图和摄像头画面的关键特征点与描述子',
        work: '调用 SIFT 算法对教材封面模板图进行特征检测，提取关键点和 128 维描述子。对摄像头实时画面逐帧进行同样的 SIFT 特征检测。对比模板和画面中特征点的分布情况。',
        problems: '最初检测到的特征点数量太多，影响后续匹配速度',
        solutions: '调整 SIFT 的 nfeatures 参数和 contrastThreshold，控制特征点数量在合理范围',
      },
      {
        title: 'FLANN 特征匹配',
        target: '对两组特征描述子进行快速匹配并筛选优质匹配',
        work: '初始化 FLANN 匹配器（使用 KD-Tree 索引），对模板和画面的特征描述子进行匹配。使用 k-NN 匹配（k=2），通过 Lowe ratio test 筛选优质匹配点，剔除模糊匹配和错误匹配。',
        problems: '匹配阈值设置困难——太高匹配点太少，太低误匹配太多',
        solutions: '采用 Lowe ratio test（距离比率 < 0.7），反复测试找到适合本场景的阈值',
      },
      {
        title: '单应性变换与目标定位',
        target: '计算变换矩阵，绘制定位框标记目标位置',
        work: '根据筛选后的匹配点对，使用 findHomography() 计算模板到画面的单应性变换矩阵。将模板的四个角点坐标通过变换矩阵映射到画面坐标，用 polylines() 绘制矩形定位框并在框上标注目标名称。',
        problems: '定位框有时偏移严重，尤其是光照变化大的时候',
        solutions: '添加图像预处理步骤（直方图均衡化），增强特征稳定性；增加最小匹配点数量检查，匹配点不足时不绘制定位框',
      },
      {
        title: '测试不同场景下的识别效果',
        target: '验证算法在不同条件下的稳定性',
        work: '在多种场景下测试：正常光照、较暗环境、画面抖动、角度偏移、部分遮挡等。记录各场景下的特征点数量、匹配精度和定位框稳定性。',
        problems: '光线过暗时特征点提取数量明显减少，匹配精度下降',
        solutions: '增加亮度自适应预处理，但效果有限——这是 SIFT 本身的局限，低光环境特征确实更难提取',
      },
      {
        title: '整理结果与撰写实验报告',
        target: '汇总测试结果，输出实验报告',
        work: '截图保存各测试场景的处理效果，整理特征检测、匹配和定位的中间结果。总结 SIFT + FLANN + 单应性变换组合的使用经验和参数配置。',
      },
    ],
    challenges: [
      {
        question: 'SIFT 特征点数量太多影响匹配速度，如何控制？',
        solutions: [
          '设置 nfeatures 参数限制最大特征点数量（如 500 个）',
          '调整 contrastThreshold 过滤低对比度的不稳定特征',
          '实现在保证匹配质量的前提下减少特征点数量',
        ],
      },
      {
        question: 'FLANN 匹配阈值如何设置才能平衡匹配数量与精度？',
        solutions: [
          '采用 Lowe ratio test，距离比率阈值设为 0.7 左右',
          '阈值 < 0.5 匹配点太少，定位框出不来',
          '阈值 > 0.8 误匹配过多，定位框不稳定会乱跳',
          '经过多轮测试，0.65-0.75 是比较实用的范围',
        ],
      },
      {
        question: '光照变化导致定位框偏移，如何缓解？',
        solutions: [
          '添加图像预处理：直方图均衡化增强对比度',
          '增加最小匹配点数检查（如至少 10 个优质匹配才绘制定位框）',
          '配合 RANSAC 算法（findHomography 默认使用），进一步剔除异常匹配',
        ],
      },
    ],
    overviewSections: [
      {
        title: '实验背景',
        content: '本实验是《VR 平台开发基础》课程的第四次实验，主题为图像特征检测。实验目的是掌握基于 SIFT 算法的图像特征检测、FLANN 特征匹配及单应性变换的目标定位方法。实验要求使用 OpenCV 完成教材封面在摄像头实时画面中的识别与框选标记。这是计算机视觉中经典的目标检测入门实践。',
      },
      {
        title: '实验流程',
        content: '实验分为四个步骤：第一步，配置 OpenCV 开发环境（含 contrib 模块以支持 SIFT），导入教材封面模板图和摄像头实时画面；第二步，调用 SIFT 算法对模板和画面分别进行特征检测，提取关键特征点与 128 维描述子；第三步，使用 FLANN 匹配器对两组特征描述子进行快速匹配，通过 Lowe ratio test 筛选优质匹配点；第四步，基于匹配点计算单应性变换矩阵，将模板轮廓坐标映射到摄像头画面中，绘制矩形定位框完成目标标记。',
      },
      {
        title: '测试结果',
        content: '实验成功实现了教材封面的实时识别与框选定位。SIFT 算法能稳定提取特征，FLANN 匹配效率较高，单应性变换可精准还原目标位置。在画面抖动、角度偏移场景下仍可正常识别，仅在光线过暗时特征点提取数量减少，匹配精度小幅下降，整体满足实验要求。',
      },
      {
        title: '实验总结',
        content: '通过本次实验，我掌握了基于 SIFT 算法的图像特征检测、FLANN 特征匹配及单应性变换的核心原理与实操方法。实验中深刻体会到参数调试的重要性——匹配阈值的设置直接影响识别效果。同时通过解决定位框偏移等问题，锻炼了问题排查与代码调试能力。代码框架部分参考了 AI 辅助，核心算法理解和参数调试由自己独立完成。',
      },
    ],
    mediaItems: [
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/feature-detection/result-1.png', type: 'image', title: 'SIFT 特征检测结果', description: '教材封面模板图的 SIFT 关键特征点提取结果' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/feature-detection/result-2.png', type: 'image', title: 'FLANN 特征匹配', description: 'FLANN 匹配器对模板与画面特征进行匹配连线展示' },
      { src: 'https://6261-bakz-portfolio-d4g00x16257e34cf7-1457429902.cos.ap-shanghai.myqcloud.com/bakz-portfolio/media/feature-detection/result-3.png', type: 'image', title: '目标定位结果', description: '单应性变换实现教材封面实时识别与矩形框选标记' },
    ],
  },
]
